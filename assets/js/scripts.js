...
var woo_address_book_app = {
	...
	checkout: function () {
		...
		if (
			$("#woocommerce_enable_shipping_calc").val() === "yes" &&
			$("form[name=checkout]").length > 0 &&
	...
	checkout_field_prepop: function (address_type, initial_address) {
		...
		if (name !== undefined && name !== null) {
			if ("add_new" === name) {
				...
				$(".woocommerce-" + address_type + "-fields__field-wrapper input")
				...
				$(".woocommerce-" + address_type + "-fields__field-wrapper select")
					.not($("#" + address_type + "_country"))
					.not("#" + address_type + "_address_book")
					.each(function () {
						var input = $(this);
					...
					success: function (response) {
						...
						if (
							...
							if (
								...
								$("input#shipping_address_book_add_new")
									.prop("checked", true)
									.trigger("change");
								return;
							}
						...
						$(document.body).trigger("address_book_changed");
